...
#    mask_int = initializers.Constant(1)
  ...
  def pconv_re_block(self,X1,X2,n_feat,k_size,act,int_nor,stride,pad,mask_int,dr_rate=0.0) :
    ...
    pconv_resnet,_ = self.pconv(X1,X2,n_feat,k_size,act_fun,int_nor,stride,pad,mask_int)
  
    pconv_resnet = layers.Dropout(dr_rate)(pconv_resnet)
  
    return pconv_resnet
  
  def de_dense(self,X,dr_rate=0.0) :
